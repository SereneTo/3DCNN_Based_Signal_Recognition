function APSK32(fc,Rs,Bw,gamma1,gamma2,times)
close all
% fc = 20e6; Rs = 1e6; Bw = 1e6; 
% gamma1=2;gamma2=3; times = 4;
r1=1; r2=gamma1*r1;r3=gamma2*r1; radii=[r1 r2 r3];
 M=[4 12 16]; modOrder=sum(M); 
n = 200;
fs = 3*fc; 
if fc == 0
    up = 8;down = 1;
...
for i = 1:times    
    ...
    apsk32data(i).data = downsample(y,down);
    commparameter(i).data = {type,fc,fs,Bw};
    %%intermediate frequency
    L = length(apsk32data(i).data);
    if fc ~= 0
        ts = 1/fs;        
        t = 0:ts:(L-1)*ts;
        apsk32data(i).data = apsk32data(i).data.*exp(1j*2*pi*fc*t);
    end
...
if fc ~= 0
    figure(3);
    subplot(311);plot(real(apsk32data(i).data))
    title('ÖÐÆµ')
    subplot(312); plot(f/1e6,fftshift(abs(fft(apsk32data(i).data)))/L*2);
    xlabel('frequncy(MHz)');
    subplot(313);spectrogram(apsk32data(i).data,256,250,256,Rs*up/down,'centered','yaxis')
end
